...
"""Bounded play rollout that reports per-env root displacement + reward.

A play.py derivative used to VERIFY a trained locomotion policy actually moves
(e.g. the StiffGIPC ant: 32 envs, 400 steps -> per-env torso displacement,
walked-env count, mean reward). Run exactly like play.py; prints DISP_RESULT.
"""
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    with launch_simulation(env_cfg, args_cli):
        ...
        timestep = 0
        # [disp-eval] bounded rollout with torso displacement + reward tally
        EVAL_STEPS = 400
        ...
        try:
            for _step in range(EVAL_STEPS):
                with torch.inference_mode():
                    ...
                    rew_sum = rew.clone() if rew_sum is None else rew_sum + rew
                    if version.parse(installed_version) >= version.parse("4.0.0"):
                        policy.reset(dones)
                    else:
                        policy_nn.reset(dones)
                if (_step + 1) % 100 == 0:
                    d = (robot.data.root_pos_w[:, :2] - p0).norm(dim=1)
                    print(f"[disp-eval] step {_step+1}: mean_disp={d.mean():.3f} m", flush=True)
            d = (robot.data.root_pos_w[:, :2] - p0).norm(dim=1)
            import json as _json
            walked = (d > 0.5).sum().item()
            print("DISP_RESULT " + _json.dumps({
                "steps": EVAL_STEPS,
                "mean_disp_m": round(float(d.mean()), 3),
                "max_disp_m": round(float(d.max()), 3),
                "walked_envs_gt0.5m": int(walked),
                "num_envs": int(d.shape[0]),
                "mean_reward_per_step": round(float(rew_sum.mean()) / EVAL_STEPS, 3),
            }), flush=True)
            env.close()
